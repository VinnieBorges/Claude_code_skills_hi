...
def unload_ollama_model(model_name):
    ...
    try:
        ...
        print(f"Warning: Failed to unload Ollama model {model_name}: {e}")
...
def run_vision_analysis(video_path, model_name="llama3.2-vision:latest"):
    ...
    """
    # Unload Gemma-4 to clear VRAM before loading Llama 3.2 Vision
    unload_ollama_model("gemma4:26b")
    ...
    try:
        ...
        for idx, fpath in enumerate(selected_frames):
            ...
            try:
                ...
                print(f"Warning: Failed to analyze keyframe at {timestamp}s: {e}")
...
def run_audio_transcription(video_path):
    ...
    # Load Whisper Model (try GPU/CUDA first, fallback to CPU)
    model = None
    try:
        ...
        import traceback
        traceback.print_exc()
        print(f"Warning: CUDA initialization or transcription failed ({e}). Falling back to CPU.")
...
def render_final_cuts(project_id, filename, original_video_path, segments, style_preset, segments_map, order=None):
    ...
    for dur in durations:
        ...
        try:
            ...
            print(f"Error copying raw cut to auto_cuts: {cp_err}")
    ...
    if order:
        ...
        try:
            ...
            print(f"Error rendering custom reordered cut: {e}")
        ...
        try:
            ...
            try:
                ...
                print(f"Error copying custom raw cut to auto_cuts: {cp_err}")
        except Exception as e:
            print(f"Error rendering custom reordered raw cut: {e}")
